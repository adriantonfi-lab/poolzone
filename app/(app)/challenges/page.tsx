...
import { Zap, Plus, Trophy, Clock, ArrowLeft, X, ChevronRight, Share2 } from 'lucide-react'
...
function CreateBattleModal({ onClose, onCreated, userId }: { onClose: () => void; onCreated: () => void; userId: string }) {
  ...
  async function handleCreate() {
    if (!selectedMatch || !prediction || !amount) {
      setError('Elegí un partido, equipo y monto')
      return
    }
    ...
    <div className="fixed inset-0 bg-black/80 z-50 flex items-end md:items-center justify-center p-0 md:p-4">
      <div className="bg-[#1A1A2E] w-full md:max-w-lg md:rounded-2xl rounded-t-2xl border border-[#2A2A4A] max-h-[90dvh] flex flex-col">
        ...
        <div className="overflow-y-auto flex-1 p-4 space-y-4">
          ...
          <div>
            <p className="text-base font-bold text-white mb-2">Elegí un partido</p>
          ...
          {selectedMatch && (
            <div>
              <p className="text-base font-bold text-white mb-2">Elegí tu equipo</p>
          ...
          {prediction && (
            <div>
              <p className="text-base font-bold text-white mb-2">Monto</p>
...
function BattleCard({ battle, userId, onRefresh }: { battle: Battle; userId: string; onRefresh: () => void }) {
  ...
  return (
    <>
      ...
      {showJoin && <JoinModal battle={battle} userId={userId} onClose={() => setShowJoin(false)} onJoined={onRefresh} />}
    </>
  )
}
...
export default function BattlesPage() {
  ...
  async function loadBattles(uid: string) {
    const supabase = createClient()
    const { data } = await supabase.from('battles').select('*').order('created_at', { ascending: false })
    setBattles(data || [])
    setLoading(false)
  }
  ...
  return (
    <div className="px-4 py-6 max-w-2xl mx-auto md:max-w-4xl">
      ...
      {showCreate && <CreateBattleModal userId={userId} onClose={() => setShowCreate(false)} onCreated={() => loadBattles(userId)} />}
    </div>
  )
}
