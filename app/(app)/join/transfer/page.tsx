...
const METHODS = [
  { id: 'zelle', flag: '🇺🇸', name: 'Zelle', detail: 'poolzone@gmail.com', note: 'USA only' },
...
function TransferContent() {
  ...
  async function handleConfirm() {
    ...
    await fetch('/api/payments/deposit', {
      ...
      body: JSON.stringify({ userId, amount: 30, paymentMethod: 'transfer', notes: 'Manual transfer pending' }),
    })
    setSent(true)
    setLoading(false)
  }
  ...
  if (sent) return (
    <div className="min-h-screen bg-[#080812] flex items-center justify-center p-4">
      <div className="text-center max-w-sm">
        ...
        <p className="text-gray-400 mb-6">We will verify your payment and activate your account within 24 hours.</p>
  ...
  return (
    <div className="min-h-screen bg-[#080812] p-4 pb-24">
      <div className="max-w-md mx-auto pt-8">
        ...
        <h1 className="text-2xl font-black text-white mb-2">Pay via Transfer</h1>
          ...
          {METHODS.map(m => (
            <div key={m.id} className="bg-white/5 border border-white/10 rounded-2xl p-4">
              ...
              <div className="flex items-center justify-between bg-black/30 rounded-xl px-3 py-2">
                ...
                <button onClick={() => copy(m.detail, m.id)} className="text-gray-400 hover:text-white">
        ...
        <p className="text-center text-xs text-gray-500 mt-4">Account activated within 24 hours after verification.</p>
      </div>
    </div>
  )
}
