...
export default async function WalletPage() {
  ...
  const [profileRes, transactionsRes, battlesRes, oracleRes] = await Promise.all([
    ...
    supabase.from('battle_participants').select('*, battles(title, status, pot_total)').eq('user_id', user.id),
  ...
  const battlesCount = battlesRes.data?.length || 0
  const oracleSpent = oracleRes.data?.reduce((s, q) => s + (q.cost || 0), 0) || 0

  // Calcular pozo total estimado (para mostrar el premio potencial)
  const { count: totalUsers } = await supabase.from('profiles').select('*', { count: 'exact' })
  const estimatedPot = (totalUsers || 0) * 25
  ...
  return (
    <div className="px-4 py-6 max-w-2xl mx-auto pb-24 md:pb-6">
      ...
      <h1 className="font-bebas text-5xl text-white tracking-wider mb-1">Mi Wallet</h1>
      <p className="text-sm font-semibold text-[#86EFAC] mb-6">@{profile?.username} · Movimientos y saldo</p>

      {/* Saldo principal */}
      <div className="bg-gradient-to-br from-[#1A1A2E] to-[#0D0D1A] border border-[#FFD700]/30 rounded-2xl p-6 mb-6">
        <p className="text-sm font-bold text-white uppercase tracking-wider mb-1">Saldo actual</p>
        <p className="font-bebas text-7xl text-[#FFD700] leading-none">{profile?.credits || 0}</p>
        <p className="text-base font-bold text-white mt-1">Créditos</p>
      ...
      <div className="grid grid-cols-2 gap-3 mb-6">
        ...
        <div className="bg-[#1A1A2E] border border-[#FFD700]/20 rounded-2xl p-4">
          <div className="flex items-center gap-2 mb-2">
            ...
            <p className="text-xs font-bold text-white">Premio potencial</p>
          </div>
        ...
        <div className="flex items-center justify-between">
          <div>
            <p className="text-base font-bold text-white">
              {profile?.inscription_status === 'paid' ? '✅ Inscripción pagada' : '⏳ Inscripción pendiente'}
            </p>
            <p className="text-xs text-gray-400">Cuota de participación: ${profile?.inscription_fee || 25}</p>
          </div>
          {profile?.inscription_status !== 'paid' && (
            <Link href="/inscription"
              className="bg-[#FFD700] text-black font-bold px-4 py-2 rounded-xl text-sm hover:bg-[#FFA500] transition-all">
              Pagar
            </Link>
          )}
        </div>
      ...
      <div className="bg-[#1A1A2E] border border-[#2A2A4A] rounded-2xl overflow-hidden">
        <div className="px-4 py-3 border-b border-[#2A2A4A]">
          <p className="font-bebas text-xl text-white tracking-wider">Historial de movimientos</p>
        </div>

        {transactions.length === 0 ? (
          <div className="px-4 py-12 text-center">
            <DollarSign size={32} className="text-gray-600 mx-auto mb-2" />
            <p className="text-white text-base">No hay movimientos todavía.</p>
          </div>
        ) : (
            ...
            {transactions.map(t => (
              <div key={t.id} className="flex items-center gap-3 px-4 py-3">
                <div className={`w-9 h-9 rounded-xl flex items-center justify-center shrink-0 ${
                  t.amount > 0 ? 'bg-[#22C55E]/20' : 'bg-red-500/20'
                }`}>
                  {getIcon(t.type)}
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-bold text-white truncate">{t.description || getTypeLabel(t.type)}</p>
                  <p className="text-xs text-gray-400">{formatDate(t.created_at)}</p>
                </div>
                <div className="text-right shrink-0">
                  <p className={`font-bebas text-xl ${t.amount > 0 ? 'text-[#22C55E]' : 'text-red-400'}`}>
                    {t.amount > 0 ? '+' : ''}{t.amount} CR
                  </p>
                  <p className="text-xs text-gray-500">Saldo: {t.balance_after}</p>
                </div>
              </div>
            ))}
          </div>
